...
def reward_func_acc(correct):
    if correct:
        return 1.0
    else:
        return -1.0

def reward_func_acc_and_parallel(correct, num_parallel, format_correct):
...
def compute_score(
    ...
    # every 10 step -> 8 step acc 2 step acc + parallel
    if ((global_step - 1) % 10) < 0.5*10:
        reward = reward_func_acc(correct)
    else:
        reward = reward_func_acc_and_parallel(correct, num_parallel, format_correct)
    return {
        "score": reward,
        "acc": correct,
        "pred": pred,
