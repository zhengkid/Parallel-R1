...
def reward_func(correct, format_ok, n_par):
    R_acc = 1.0 if correct else -1.0        
    R_fmt = 0.3 if format_ok else -0.3       
    gate  = 1.0 if (correct and format_ok) else 0.0

    R_par = 0.6 * math.log1p(n_par)           

    R_par_wrong = -0.1 * math.log1p(n_par) if (not correct and n_par>0) else 0.0

    return R_acc + R_fmt + gate * R_par + R_par_wrong
...
def compute_score(
    ...
    if num_parallel >= 1 and format_correct==1:
        parallel_reward = 0.5
    else:
        parallel_reward = 0
    reward = 1.0 + parallel_reward if correct else -1.0 + parallel_reward
    acc = correct
