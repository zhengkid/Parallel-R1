...
class AgentLoopOutput(BaseModel):
    ...
    position_required_mask: list[tuple[int, int, int, int, int]] | None = None
    num_turns: int = 0
...
class AgentLoopWorker:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        if "agent_name" not in batch.non_tensor_batch:
            batch.non_tensor_batch["agent_name"] = np.array(["parallel_thinking_agent_v3"] * len(batch), dtype=object)
    ...
    def _postprocess(self, inputs: list[AgentLoopOutput]) -> DataProto:
        ...
        # print("response_mask after ", response_mask)
        input_ids = torch.cat([prompt_ids, response_ids], dim=1)
        # print("debug response mask", response_ids[response_mask])
        # print("debug response mask", response_ids[response_attention_mask])
        attention_mask = torch.cat([prompt_attention_mask, response_attention_mask], dim=1)
        # position_ids = (attention_mask.cumsum(dim=1) - 1) * attention_mask
        position_ids = torch.stack([it.multiverse_pos_ids for it in inputs], dim=0)   
        batch = TensorDict(
            {
                ...
                "final_attention_mask": attention_mask,  # [bsz, prompt_length + response_length]
                "position_ids": position_ids,  # [bsz, prompt_length + response_length]
            },
            batch_size=len(input_ids),
        )

        left_pad_lens = [self.config.actor_rollout_ref.rollout.prompt_length - len(input.prompt_ids) for input in inputs]

        def _to_list_of_lists(x):
            """把 None / [] / [int,...] / [[int,...], ...] 统一成 List[List[int]]"""
            if x is None:
                return []
            if isinstance(x, (list, tuple)) and all(isinstance(t, (int, np.integer)) for t in x):
                return [list(map(int, x))]
            out = []
            for p in x:
                out.append([int(v) for v in p])   # tuple/np.int64 -> int
            return out

        batch_size = len(inputs)
        position_required_masks_all = np.empty(batch_size, dtype=object)  # ndarray 要求

        for i in range(batch_size):
            prm = getattr(inputs[i], "position_required_mask", None)
            norm = _to_list_of_lists(prm)   # [] 或 [[...],[...]] 都被规范化
            position_required_masks_all[i] = norm
