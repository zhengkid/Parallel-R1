            ...
            if self.use_remove_padding:
                ...
                if "position_required_masks" in micro_batch.keys():
                    ...
                    for i in range(batch_size):
                        ...
                        left_pad_len_i = left_pad_lens[i]
                        # print(position_required_masks_i)
                        for position_required_mask in position_required_masks_i:
                            if len(position_required_mask) == 0:
                                continue
                            left_pad_len, path1_start, path1_end, path2_start, path2_end = position_required_mask
                            ...
                            # )
                            # the </Parallel> should be the max lenth + 1
                            # max_path_end = max(path1_end, path2_end)
                            # assert position_ids[i][max_path_end] - position_ids[i][path1_end-1] == 1 or position_ids[i][max_path_end] - position_ids[i][path2_end-1] == 1, (
                            #     f"position_ids {position_ids[i][max_path_end]} - {position_ids[i][path1_end-1]} != 1 or {position_ids[i][max_path_end]} - {position_ids[i][path2_end-1]} != 1 for input {i}"
                            # )
                            # path1 and path 2 should be <Path> and </Path>
                            # path1 and path 2 should be </Path>
                            
                            # assert input_ids[i][path1_end-1] == 151670 and input_ids[i][path2_end-1] == 151670, (
                            #     f"input_ids {input_ids[i][path1_end-1]} != 151670 or {input_ids[i][path2_end-1]} != 151670 for input {i}"
                            # )
                            # if path1_end > path2_end: 
                            #     # print(position_ids[i])
                            #     # print(input_ids[i])
                            #     assert input_ids[i][path1_start] == input_ids[i][path2_start] and input_ids[i][path1_start] == 151669 and input_ids[i][path2_start] == 151669, (
                            #         f"input_ids {input_ids[i][path1_start]} != 151669 or {input_ids[i][path2_start]} != 151669 for input {i}"
                            #     )
                            #     assert position_ids[i][path1_start] == position_ids[i][path2_start], (
                    ...
                    # attention_mask = attention_mask.unsqueeze(1)  # (bsz, 1, seqlen, seqlen)
